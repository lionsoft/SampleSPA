...
(function (st) {
    ...
    st.format.add = function (name, func, replace) {
        if (formatters[name] != null && replace != true)
    ...
    st.format.add('arr', function arr(va, params) {
        ...
        if (f == null)
            ...
            for (var i = 0; i < va.length; i++)
                v.push(f(va[i], params)); // Применяем формат к каждому элементу
...
(function (format) {
    ...
    format.add('n', function n(v, params) {
        if ((v = numformat.exec(v)) == null)
            return defaults.nan; // Уточняем момент насчет числа
    ...
    function addz(pre, v, l) {
        if (isNaN(l = Number(l == '' ? undefined : l)))
            return (v == null || v == '') ? '' : ((pre || '') + v); // Нет ограничений - просто вернем то, что есть
        if ((v = v || '').length >= l)
            return pre == null ? v : (pre + v.substr(0, l)); // Значение больше нужного - целую часть оставляем, дробную режем
...
(function (format) {
    ...
    format.add('ts', function ts(v, params) {
        ...
        if (v != 0)
            s.unshift(v, tsdefault.d); // Не забываем вывести дни, если они есть
    ...
    function df(v, p) {
        ...
        if (p == '')
            return v; // Нет формата - делать нечего
    ...
    function tss(dt, div, buf, msn, write) {
        ...
        if (i != 0 && write)
            buf.unshift(i, msn); // Выводим, если нужно
        return Math.floor(dt / div); // Вернем секвестированное число
    }
})(String.prototype.format);
//# sourceMappingURL=string.format-1.0.js.map